...
import (
	...
	"test/log"
	"test/pkg/sip"
	"test/pkg/sip/transaction"
	"test/pkg/sip/transport"
...
type TransactionLayerFactory func(tpl sip.Transport, logger log.Logger) transaction.TransactionLayer
...
type server struct {
	...
	tx              transaction.TransactionLayer
	host            string
	ip              net.IP
	hwg             *sync.WaitGroup
	hmu             *sync.RWMutex
...
func NewServer(
	...
	if tpFactory == nil {
		tpFactory = transport.NewTransportLayer
	}
	if txFactory == nil {
		txFactory = transaction.NewTransactionLayer
	}
	...
	if config.Host != "" {
		...
		if v, err := lib.ResolveSelfIP(); err == nil {
			ip = v
			host = v.String()
		} else {
			logger.Panicf("resolve host IP failed: %s", err)
